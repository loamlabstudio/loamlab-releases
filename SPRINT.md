...
> **定價稽核（2026-09-11，無版本號 — 未動到出貨程式邏輯）**
> 
> 用戶回報「官網和軟體上的定價點數不同」。Gemini 的 `SPRINT_PROPOSAL.md` 判定為
> 兩處前端顯示錯誤，查證後**兩個 Phase 都不能照做**（詳見該檔，status 已改 REJECTED）。
> 
> **官網從頭到尾是對的**；真正的錯在 Dodo 後台 Top-up 標價設成 $13（應 $18），
> 已由用戶修正。提案指定要改的 `loamlab_camera_official.html` 是 2026-07-17 的死檔，
> `vercel.json` 只 rewrite 到 `public/index.html`，照做會把正確的官網改壞。該死檔本次刪除。
> 
> 唯一屬實的 bug 是 `i18n.js` 的 Pro 1,000 / Studio 3,000 覆蓋了 `index.html` 裡正確的
> 2,000 / 9,000，已修（6 語系 + 編譯檔）。提案要跑的 `scripts/sync_i18n.js` 不存在，
> 且 `locales/*.json` 根本沒被載入，只改 JSON 是零效果——兩邊都得手改。
> 
> **根因**：`POINTS_SYSTEM.md` 記載的「原價 $25/$35/$75/$199」從未在 Dodo 或任何程式碼
> 出現過，是 2026-03-20 初版的紙上錨點。現行掛牌價由它 × 0.7 反推而來、四個數字剛好吻合，
> 導致任何人讀到都會誤判「掛牌價已是折後價」而把折扣重算一次。Gemini 栽在這，第一輪稽核也栽了。
> `POINTS_SYSTEM.md` 已改寫為掛牌價／顯示價／實收三層定義，並附手動同步清單。
> 
> 刻意**不建**定價自動同步機制：一年改不了幾次，且插件離線時必須有本地值可顯示。
