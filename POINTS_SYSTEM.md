...
為了將「用戶口碑」轉化為「永久的增長飛輪」，我們導入裂變獎勵機制。新用戶（B）首次算圖成功 +100 pts，邀請人（A）+300 pts。
...
3. **無感觸發 (Zero-Friction)**：新用戶**首次成功算圖**時，`render.js` 自動偵測綁定關係，背景無聲派發 **+100 pts 給新用戶（B）**、**+300 pts 給邀請人（A）**，並標記 `referral_rewarded = true` 防重複。
