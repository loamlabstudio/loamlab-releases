...
- **套用方式**：`loamlab_backend/api/user.js` 對**每一筆** checkout 自動帶入（`DODO_DISCOUNT_CODE`），
  用戶無需手動輸入。若 Dodo 拒絕該碼，後端會自動去掉折扣重試一次。
- **Beta 用戶身份**：公測期付費用戶標記為 Beta Tester，享有首年或永久折扣承諾。
  訂閱金額鎖定在建立當時，日後若停用折扣碼，既有訂閱者續訂金額不受影響。

#### ⚠️ 三層價格定義（2026-09-11 校正，勿再混用）

| 層級 | 定義 | 數值 |
|---|---|---|
| **掛牌價** | Dodo 後台商品標價；也是官網／插件劃掉的那個數字 | $18 / $24 / $52 / $139 |
| **顯示價** | 前端算給用戶看的折後價 `Math.round(掛牌價 × 0.7)` | $13 / $17 / $36 / $97 |
| **實收** | 折扣碼自動套用後真正入帳 `掛牌價 × 0.7` | $12.60 / $16.80 / $36.40 / $97.30 |

顯示價因四捨五入與實收最多差 $0.40（Pro 顯示 $36、實收 $36.40）。

> **關於「原價 $25 / $35 / $75 / $199」**：舊版本文件曾把這組數字記為原價，
> 但它**從未在 Dodo 後台或任何前端程式碼出現過**，是 2026-03-20 初版規劃階段的錨點
> （當時連支付平台都還沒選定，原文寫的是 LemonSqueezy 或 Stripe）。
> 現行掛牌價正是由它 × 0.7 反推而來（$75×0.7≈$52、$35×0.7≈$24、$199×0.7≈$139、$25×0.7≈$18），
> 四個數字剛好吻合，極易讓人誤判「掛牌價已經是折後價」而把折扣重複計算一次。
> **它不是現行定價，不可再引用。**

### 單次購買包 (Top-up)
- **掛牌價**: `$18`　→　**實收**: `$12.60`（約 `NT$ 415`）
...
| 方案等級 | 方案名稱 | 掛牌價 (Dodo) | 實收 (自動 -30%) | 每月發放點數 | 換算張數 (以2K計) |
| -------- | -------- | ------------- | ---------------- | ------------ | ---------------- |
| 基礎 | **Starter** | $24 | **$16.80** | **300** | 15 張 |
| 專業 | **Pro** | $52 | **$36.40** | **2,000** | 100 張 |
| 工作室 | **Studio** | $139 | **$97.30** | **9,000** | 450 張 |

> **本表是定價與點數的唯一真實來源。** 沒有自動同步機制（刻意不建：定價一年改不了幾次，
> 且插件離線時必須有本地值可顯示）。改動時手動同步下列各處：
>
> | 改什麼 | 要同步的地方 |
> |---|---|
> | 掛牌價 | Dodo 後台商品 → `loamlab_backend/public/index.html` 的 `PricingConfig.origPrice/price` → `loamlab_plugin/ui/index.html` 的 `data-original-price` |
> | 每張成本換算 | `PricingConfig.exchText` ↔ `loamlab_plugin/ui/app.js` 的 `PLAN_RENDER_COST_USD` |
> | 發放點數 | `loamlab_plugin/ui/locales/*.json`（6 檔）**以及**編譯產物 `loamlab_plugin/ui/i18n.js`；沒有編譯腳本（`scripts/sync_i18n.js` 不存在），兩邊都要手改，否則 `i18n.js` 會覆蓋 HTML 裡的預設值 |
> | 出圖張數 | `PricingConfig.rendersHint` ↔ `app.js` 的 `PLAN_RENDER_COUNTS` |
...
> **範圍限定：只有 Tool 1/2/3 的 AI 算圖（`render.js` 的 `saveRenderHistory`）會觸發免費層，360 全景分享（Tool 4）刻意排除。** 360 是本機操作（把既有截圖包裝成可分享連結），不呼叫 AtlasCloud，我方零 API 成本，因此 360 本身**完全免費（0 點）**——2026-07-23 已從原本誤收的 5 點修正為 0 點。若 360 這種零成本操作還能觸發免費層邀請獎勵，等於零門檻套利，問題比「低成本」更嚴重，故排除。
