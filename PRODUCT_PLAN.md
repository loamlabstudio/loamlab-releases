...
| 優先級 | 任務 | 理由 | 對應章節 |
|--------|------|------|----------|
| P1 | 分享碼系統實作 | 提升用戶裂變率，機制已定稿 | §3 |
| P2 | 工具 3（九宮格鏡頭） | 核心 Wow Effect 功能 | §5 |
...
## 3. 📋 分享碼系統（Referral System）

**策略目標**：透過雙向獎勵提升用戶裂變率。

| 角色 | 觸發條件 | 獎勵 |
|------|----------|------|
| 邀請人 A | B 完成首次渲染後 | +300 點 |
| 受邀人 B | 完成首次渲染後 | +100 點（限一次） |

> ⚠️ **優化注意**：受邀人 B 的 50 點獎勵應在首次渲染完成後才發放，而非輸入邀請碼時立即發放，以防「創帳號即領點」的濫用行為。`referral_rewarded` 欄位已存在，可作為判斷依據，無需新增 DB 欄位。
